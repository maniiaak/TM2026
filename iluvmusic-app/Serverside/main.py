...
def get_album_reviews(album_id):
    ...
    # 1. Get the list of reviews (include user_id for profile navigation)
    cursor.execute("""
    ...
    # 3. Format Reviews List (include user_id for profile navigation)
    reviews_list = []
    for row in reviews:
        reviews_list.append({
            "rating": row['rating'],
...
def spotify_login():
    ...
    print(f"  Redirect URI: com.maniiaak.iluvmusic://callback")
    ...
    body = {
        ...
        "code": code,
        "redirect_uri": "com.maniiaak.iluvmusic://callback",
        "client_id": client_id,
